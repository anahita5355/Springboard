...
SELECT m1.surname, m1.firstname,m1.recommendedby AS recommender_id, CONCAT(m2.surname, ' ', m2.firstname) AS recommender
FROM Members AS m1
...
SELECT name, COUNT(memid) AS mem_usage
FROM Bookings
...
SELECT MONTH(starttime) AS month, COUNT(memid) AS mem_usage
FROM `Bookings`
WHERE memid <>0
GROUP BY month
